...
Scope includes all runtime flows: workflow, query/UI, indexing, metadata-only, async queue, and legacy CLI.
...
1. Workflow orchestration
   - `prep` -> `ingest` -> `enrich` -> optional `econ_data` -> optional `agentic` -> optional `index` -> `evaluate` -> `report` in `ragonometrics/pipeline/workflow.py:823`.
2. Ingestion + enrichment coupling
   - `load_papers` in `ragonometrics/core/main.py:1101` performs extraction plus OpenAlex/CitEc fetching inline.
3. Retrieval path selection
   - `top_k_context` in `ragonometrics/core/main.py:828` tries Postgres hybrid retrieval first (`ragonometrics/indexing/retriever.py:138`), otherwise local embedding cosine fallback.
4. Indexing persistence
   - `build_index` in `ragonometrics/indexing/indexer.py:124` writes vectors/documents/paper metadata, index versions/shards/manifests.
...
- Legacy DOI-network schema path in `ragonometrics/core/main.py` has been removed in this branch.

## Essentialness Table (Pipeline Pieces)
| Component | Location | Function | Essentialness | Keep/Simplify Decision |
| --- | --- | --- | --- | --- |
| Workflow orchestrator | `ragonometrics/pipeline/workflow.py:823` | End-to-end run control | Critical | Keep as primary orchestrator |
| Prep profiler | `ragonometrics/pipeline/prep.py:71` | Corpus validation/manifest | High | Keep; maintain as preflight gate |
| Paper ingestion | `ragonometrics/core/main.py:1101` | PDF text + metadata extraction | Critical | Keep; split enrichment concerns |
| Chunking | `ragonometrics/core/main.py:1036` + `ragonometrics/core/io_loaders.py` | Provenance chunk generation | Critical | Keep |
| Embedding utility | `ragonometrics/core/main.py:673` | Embedding generation | Critical | Keep; centralize all embedding calls |
| Retrieval selector | `ragonometrics/core/main.py:828` | Hybrid/local context retrieval | Critical | Keep; make deterministic mode explicit |
| Agentic QA/report questions | `ragonometrics/pipeline/workflow.py` (`_answer_report_questions` at `:715`) | Deep analysis/report generation | High | Keep as optional module |
| Index builder | `ragonometrics/indexing/indexer.py:124` | Vector + metadata index creation | Critical | Keep; isolate side effects |
| Metadata schema helpers | `ragonometrics/indexing/metadata.py:70` | Postgres schema/run/index records | Critical | Keep |
| Hybrid retriever | `ragonometrics/indexing/retriever.py:138` | BM25 + vector search | High | Keep; tighten doc/paper scoping |
| Workflow state store | `ragonometrics/pipeline/state.py:54` | Step/state audit trail | High | Keep |
| Workflow report store | `ragonometrics/pipeline/report_store.py:219` | JSONB report persistence | High | Keep |
| Query cache | `ragonometrics/pipeline/query_cache.py:43` | Reuse Q/A responses | Medium | Keep optional |
| Token usage logging | `ragonometrics/pipeline/token_usage.py` | Usage observability | Medium | Keep optional |
| Streamlit app | `ragonometrics/ui/streamlit_app.py:536` | Primary interactive UX | High | Keep |
| Async queue wrapper | `ragonometrics/integrations/rq_queue.py:32` | Background workflow/index jobs | Medium | Keep optional |
...
| Legacy LLM pipeline paths | `ragonometrics/pipeline/pipeline.py` | Old summarize/rank/metadata paths | Legacy | Removed; retained active helpers only (`call_openai`, `extract_json`, `extract_citations`) |
| Legacy CLI wrapper | `ragonometrics/cli/cli.py` | Alternate CLI surface | Legacy | Removed in favor of `entrypoints.py` |
| Legacy DOI network schema path | `ragonometrics/core/main.py` DOI DB functions | Separate citation network storage | Optional/Legacy | Removed from active runtime |
